...
def plot_zipf_param_surface(empirical_freqs: np.ndarray):
    K_plot = np.linspace(-0.03, 0.05, 100)
...
def plot_zipf_param_contours(empirical_freqs: np.ndarray):
    K_plot = np.linspace(-0.03, 0.07, 100)
    alpha_plot = np.linspace(-1, -0.3, 200)
...
def plot_zipf_fit(empirical_freqs):
    ...
    plt.plot(
        ...
        c="tab:orange",
        linewidth=2,
        label=rf"$f_{{ols}}(x) = {K_ols:.2}x^{{{alpha_ols:.2}}}$",
    )
    plt.plot(
        xs,
        K_nlls * xs**alpha_nlls,
        c="tab:green",
        linewidth=2,
    ...
    plt.plot(np.log(xs), np.log(K_ols * xs**alpha_ols), c="tab:orange", linewidth=2, label=r"$\log(f_{ols}(x))$")
    plt.plot(np.log(xs), np.log(K_nlls * xs**alpha_nlls), c="tab:green", linewidth=2, label=r"$\log(f_{nlls}(x))$")
...
def plot_gauss_newton_convergence(empirical_freqs):
    ...
    for i, alpha in enumerate(alpha_plot):
        for j, c in enumerate(K_plot):
            ...
            mse = np.sqrt(np.mean(np.square(residual)))
            Z[i, j] = mse
    lvls = np.asarray([0.00 + 0.03 * i for i in range(1, 20)] + [-0.3 + 1.3**i for i in range(20)])
...
if __name__ == "__main__":
    ...
    most_freq_words = data["words"]
    freqs = data["freqs"]

    with open(DATA_DIR / "zipf-hamlet.json", "wt") as f:
