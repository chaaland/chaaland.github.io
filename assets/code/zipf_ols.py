...
# def fit_heaps_ols(n_distinct_words: np.ndarray):
#     ranks = np.arange(1, 1 + n_distinct_words.size)
#     A = np.stack([np.ones_like(n_distinct_words), np.log(ranks)], axis=1)
#     b = np.log(n_distinct_words)
#     f = lambda x: A @ x - b
#     opt_result = least_squares(f, x0=np.zeros(2))
...
# def n_distinct_words_per_doc_length():
#     with open(pjoin("..", "txt", "hamlet.txt"), "rt") as f:
#         text = " ".join(f.readlines())
#         text = re.sub("[^0-9a-zA-Z]+", " ", text)
#         counts = []
#         distinct_words = set()
#         for w in text.split(" "):
#             distinct_words.add(w.lower())
#             counts.append(len(distinct_words))
...
def plot_zipf_param_surface(empirical_freqs):
    ...
    ax.set_title(r"$\sum_{i=1}^N (f_i - C x^\alpha)^2$")
...
def plot_zipf_transformed_param_surface(empirical_freqs):
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.plot_surface(C_mesh, Alpha_mesh, Z, cmap="hot")
    ax.set_title(r"$\sum_{i=1}^N (f_i - \log(C) - \alpha \log(x))^2$")
...
def plot_zipf_transformed_param_contours(empirical_freqs):
    ...
    plt.title(r"$\sum_{i=1}^N (f_i - \log(C) - \alpha \log(x))^2$")
    plt.xlabel(r"$\log(C)$")
    plt.ylabel(r"$\alpha$")
    plt.tight_layout()
...
def plot_ols_zipf_fit(empirical_freqs, N):
    ...
    plt.ylim([0, 0.04])
    plt.legend()
    # plt.savefig(pjoin("..", "images", "shakespeare-word-frequency.png"))

if __name__ == "__main__":
    N = 100
    most_freq_words, freqs = word_freqs(N)
    # plot_scatter_points(freqs, N)
    ...
    # counts = n_distinct_words_per_doc_length()
