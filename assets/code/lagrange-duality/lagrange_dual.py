...
def plot_relaxed_primal(save_dir):
    ...
    plt.plot(
        feasible_region,
        lagrangian(feasible_region, multiplier),
        color="xkcd:purple blue",
        linestyle="--",
        label=r"$x^2 + 1 + \lambda_0 (x^2-6x+8)$",
    )

    plt.plot(
        feasible_region,
        feasible_f0,
        color="xkcd:darkish green",
        linewidth=4,
        label=r"$x^2 + 1 + \mathbf{1}_\infty\{x^2-6x+8 \leq 0\}$",
    )
