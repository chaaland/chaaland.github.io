...
<!-- TODO caseyh: is this even right?? Why does y_i appear on the RHS? Seems...odd -->
...
But by rearranging the eqauation, moving the coefficients to one side, and denoting $$y_i = f(x_i)$$, we can express the coefficients as the solution to an ordinary least squares problem
...
Notice that the remaining $$m$$ columns are _nearly_ Vandermonde except for the missing first column and the multiplation by $$-y_i$$.
...
At the postive integers $$\Gamma(n+1) = n!$$ but at the non-positive one it has asymptotes.
...
<figure class="half">
    ...
    <figcaption>Figure 5: Degree 8 and 9 OLS fits to the gamma function are quite poor</figcaption>
</figure>
...
While the least squares approach served us well for simple function like $$\log(1+x)$$, it struggles with functions like $$\Gamma(x)$$ where asymptotes and discontinuities abound.

## AAA

A paper from 2017 introduces the AAA algorithm for rational function approximation.
Rather than represent the rational function the quotient of two $$m$$ degree polynomials as we did in the previous section, the authors express the rational function in the following _barycentric form_

$$
f(x) = {n(x) \over d(x) } = {\sum_{k=1}^m {w_ky_k \over x - \tilde{x}_k } \over \sum_{k=1}^m {w_k \over x - \tilde{x}_k } }.
$$

$$\tilde{x}_k$$ are the so called _support points_ and $$w_k$$ are the coefficients to fit.
...
This point $$(\tilde{x}_1, \tilde{y}_1)$$ becomes the first element of the set of support points.
...
Since $$x\in \mathbf{R}^{N-m}$$ are just the points not in the support set, they are constant from the perspective of the inner optimisation.
We should instead view the residual as a function of $$w$$

$$
r(w) =
\begin{bmatrix}
\sum_{i=1}^m {y_1 -\tilde{y}_k \over x_1 - \tilde{x}_k} w_k \\
\sum_{i=1}^m {y_2 -\tilde{y}_k \over x_2 - \tilde{x}_k} w_k \\
\vdots \\
\sum_{i=1}^m {y_{N-m} -\tilde{y}_k \over x_{N-m} - \tilde{x}_k} w_k \\
\end{bmatrix}
$$
...
In words, we want to apply a linear map to the unit sphere in $$\mathbf{R}^{m}$$ and find a point on the sphere $$w$$ that maps to the vector with smallest magnitude.
...
In fact, technically the rational function $$f(x)$$ is undefined at the support points because of the division by zero in both the numerator and the denominator.
