...
By the end of this post, you'll see how when using a fixed point representation of decimal numbers, computing transcendental functions like sine, cosine and even the exponential can be reduced to a sequence of addition, subtraction, multiplication and bit shifts using an algorithm called coordinate rotation digital computer (CORDIC).
...
The sine and cosine are then the $$y$$ and $$x$$ coordinates of the final vector!
...
You can think of this almost like a sort of binary search.
This process is repeated either for a finite number of iterations or until some tolerance is reached.
Figure 5 shows CORDIC after 11 iterations and we can see that our vector is nearly collinear with the target vector.
...
The direction of rotation is decided by whether our current estimate of $$\theta$$ exceeds our target value.
Finally, we multiply by the gain and return the cosine and sine in a list.
...
def cordic(theta: float, n_iters = 20) -> list[float]:
